...
@dataclass
class LanternTranslateAgentConfig(AgentConfig):
    to_lang: str
    custom_prompt: str | None = None
    glossary_dict: dict[str, str] | None = None


class LanternTranslateAgent(Agent):
    def __init__(self, config: LanternTranslateAgentConfig):
        super().__init__(config)
        self.to_lang = config.to_lang
        self.system_prompt = ""
        self.custom_prompt = config.custom_prompt
        if config.custom_prompt:
            self.system_prompt += self.custom_prompt
    ...
    def _result_handler(self, result: str, origin_prompt: str, logger: Logger):
        ...
        if result == "":
            ...
            return {}
        try:
            original_chunk = json.loads(original_segments)
